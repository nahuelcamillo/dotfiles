...
#readarray -t active_outputs < <(i3-msg -t get_outputs | jq -r 'map(select(.focused == true) | .name)')
readarray -t active_outputs < <(i3-msg -t get_workspaces | jq -r '.[] | select(.focused == true) | [.output]')
